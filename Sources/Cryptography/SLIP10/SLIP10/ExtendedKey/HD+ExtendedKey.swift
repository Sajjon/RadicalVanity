...
extension HD.ExtendedKey {
	func keyAsData(forceSelectPublicKey: Bool) -> Data {
		var serializedBytes: Data = switch (forceSelectPublicKey, key) {
		case (false, let .privateKey(privateKey)):
			privateKey.rawRepresentation
		case (true, _):
			self.publicKey.compressedRepresentation
...
enum KeyToDerive: Equatable {
	...
	var isPublicOnly: Bool {
		switch self {
		case .derivePrivateKey: false
		case .derivePublicKeyOnly: true
		}
	}
}
